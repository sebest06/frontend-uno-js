...
const Play = ({ socket, code, rojo, verde, amarillo, azul}) => {
  ...
  const handleDecirUNO = () => {
    ...
    const crowns_draw = (n, ix) => {
    const icons = []
    for (let i = 0; i < n; i++) {
      icons.push(
          <svg key={(ix+1)*10+i} className="w-8 h-8 text-red-600 " aria-hidden="true" viewBox="0 0 128 128" xmlns="http://www.w3.org/2000/svg" role="img">
